/**
 * PaymentReceiptPDF.jsx  —  DocEdge Subscription Invoice
 * Uses html2canvas + jsPDF — pixel-perfect PDF matching the preview.
 *
 * Install:
 ...
 *   <PaymentReceiptPDF details={details} onClose={() => setShowReceipt(false)} />
 *
 * details shape:
 * {
 *   clinicName, planName, invoiceNumber,
 *   paymentMode, bankName, mobile, email,
 *   amount, transactionId, loginUrl,
 * }
 */
...
const autoInv = () => "INV-" + Date.now().toString().slice(-8).toUpperCase();
...
async function generatePDF(paperRef, invoiceNumber) {
  ...
  while (yOffset < imgH) {
    ...
    pdf.addImage(
      imgData, "PNG",
      0, -yOffset,          // shift image up for subsequent pages
      imgW, imgH,
      undefined, "FAST"
    );
    yOffset += pageH;
...
export default function PaymentReceiptPDF({ details = {}, onClose }) {
  const {
  ...
  const S = {
    ...
    accentBar: { height: 4, background: "linear-gradient(90deg,#2563eb,#7c3aed,#ec4899)" },
  ...
  return (
    <div style={S.root}>
      ...
      {/* paper — this div gets captured by html2canvas */}
        ...
        <div ref={paperRef} style={S.paper}>
          ...
          <div style={S.header}>
            <div>
              ...
              <div style={{ fontSize: 12, opacity: 0.65, marginTop: 3 }}>Clinic Management Software</div>
            </div>
          ...
          <div style={S.section}>
            ...
            <div style={{ display: "flex", gap: 16 }}>
              <div style={{
                ...
                {mobile   && <IL>📱 {mobile}</IL>}
                {email    && <IL>✉️  {email}</IL>}
                {loginUrl && <IL style={{ wordBreak: "break-all" }}>🔗 {loginUrl}</IL>}
              </div>
            ...
            <tbody>
              <tr>
                ...
                <TD>
                  ...
                  <div style={{ fontSize: 11, color: "#94a3b8" }}>Software subscription — clinic management platform</div>
                </TD>
...
function IL({ children, style }) {
  return <div style={{ fontSize: 13, color: "#475569", marginBottom: 4, lineHeight: 1.6, ...style }}>{children}</div>;
}
...
function TotalRow({ label, val, bold, highlight }) {
  return (
    <div style={{
      ...
      borderTop:    highlight ? "2px solid #2563eb" : "none",
      borderBottom: highlight ? "2px solid #2563eb" : "none",
